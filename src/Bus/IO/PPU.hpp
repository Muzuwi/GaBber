...
struct OBJAttr {
	...
	inline uint8 width() const { return size_lookup[attr1.size][static_cast<uint8>(attr0.obj_shape)][0]; }

	inline uint8 height() const { return size_lookup[attr1.size][static_cast<uint8>(attr0.obj_shape)][1]; }

	inline uint8 top() const { return attr0.pos_y; }

	inline uint8 bottom() const { return (attr0.pos_y + height()) % 256; }

	inline uint16 left() const { return attr1.pos_x; }

	inline uint16 right() const { return (attr1.pos_x + width()) % 512; }
	...
	inline bool is_enabled() const {
		return attr0.rot_scale ||
		       !attr0.double_size_or_disable;//  FIXME: Might be wrong (does rot_scale imply enabled?)
	}
...
protected:
	...
	void on_write(uint32 new_value) override { m_register = new_value & writeable_mask; }
...
protected:
	void on_write(uint16 new_value) override { this->m_register = new_value; }
